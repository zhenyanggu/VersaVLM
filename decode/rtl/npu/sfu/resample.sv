...
module resample_unit 
    import npu_config_pkg::*;
#(
    localparam  ADDR_WIDTH          = $clog2(SPM_SIZE)          ,
    localparam  PE_IDX              = $clog2(PE_WIDTH)          ,
    localparam  RESAMPLE_IDX_WIDTH  = $clog2(INPUT_WIDTH_MAX)   ,    // index for the image to resample, height and width limited by 2^(RF_DATA_WIDTH/8)
    ...
    logic   [RESAMPLE_IDX_WIDTH-1:0][PE_DATA_WIDTH-1:0]     temp_data_col           ;   // store temporary column data for pooling
    ...
    always_ff@(posedge clk or negedge rst_n)begin
        ...
        else if(temp_col_wr_ptr == INPUT_WIDTH_MAX || resample_done_out)
        temp_col_wr_ptr <=0;
    ...
    always_ff@(posedge clk or negedge rst_n)begin
        ...
        else if(temp_col_rd_ptr == INPUT_WIDTH_MAX || resample_done_out)
        temp_col_rd_ptr <=0;
    ...
    //-------------------------------------------------------------------
    // vector and row complete signals
    //-------------------------------------------------------------------

    // always_ff@(posedge clk or negedge rst_n)begin : vector_resample_done
    //     if(~rst_n)begin
    //         vector_resample_done <= 1'b0;
    //     end
    //     else begin
    //         if(resample_input_en | resample_done)begin
    //             vector_resample_done <= 1'b0;
    //         end
    //         else begin
    //             case(resample_type)
    ...
    //                         vector_resample_done <= 1'b0;
    //                     end
    //                     else if((row_ptr == resample_input_row_num) && (resample_input_row_num[0] == 1'b1))begin // odd row num, last row only downsample horizontally
    //                         vector_resample_done <= 1'b1;
    //                     end
    //                     else begin
    //                         vector_resample_done <= ~vector_resample_done
    //                     end
    //                 end
    ...
    function logic [PE_DATA_WIDTH-1:0] avg4to1(
        ...
        sum = ($signed(data_in[0]) + $signed(data_in[1])) + ($signed(data_in[2]) + $signed(data_in[3]));
    ...
    function logic [PE_DATA_WIDTH-1:0] max2to1(
        ...
        if ($signed(data_in[0]) >= $signed(data_in[1])) begin 
            return  data_in[0];
        end
        else begin
            return  data_in[1];
    ...
    function logic [PE_DATA_WIDTH-1:0] avg2to1(
        ...
        sum = ($signed(data_in[0]) + $signed(data_in[1]));
