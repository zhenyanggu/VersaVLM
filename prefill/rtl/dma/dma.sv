...
localparam      REAL_NUM_WIDTH       = DMA_DIM_WIDTH+DATA_IDX+1;
	...
	localparam int  AXI_MAX_BURST_LG2    = $clog2(AXI_MAX_BURST_BEATS);
	localparam int  AR_REMAIN_CHUNK_W    = BEATS_WIDTH - AXI_MAX_BURST_LG2;
...
logic    [DMA_DIM_WIDTH-1:0]                 mvin_col_num_q                  ;
logic    [DMA_DIM_WIDTH-1:0]                 mvin_row_num_q                  ;
logic    [DMA_DIM_WIDTH-1:0]                 mvout_col_num_q                 ;
logic    [DMA_DIM_WIDTH-1:0]                 mvout_row_num_q                 ;
logic    [RF_DATA_WIDTH/4-1:0]               cfg_mvin_sram_stride_q          ;
logic    [RF_DATA_WIDTH/2-1:0]               cfg_mvin_dram_stride_q          ;
logic    [RF_DATA_WIDTH/4-1:0]               cfg_mvout_sram_stride_q         ;
logic    [RF_DATA_WIDTH/2-1:0]               cfg_mvout_dram_stride_q         ;
logic    [REAL_NUM_WIDTH-1:0]                mvin_rows_real_q                ;
logic    [REAL_NUM_WIDTH-1:0]                mvin_cols_real_q                ;
logic    [REAL_NUM_WIDTH-1:0]                mvout_rows_real_q               ;
logic    [REAL_NUM_WIDTH-1:0]                mvout_cols_real_q               ;
logic    [REAL_NUM_WIDTH-1:0]                mvin_contiguous_elems_q         ;
logic    [REAL_NUM_WIDTH-1:0]                mvout_contiguous_elems_q        ;
logic                                        mvin_contiguous_2d_q            ;
logic                                        mvout_contiguous_2d_q           ;
...
logic    [BEATS_WIDTH-1:0]                   ar_row_beats_calc               ;
logic    [AR_REMAIN_CHUNK_W-1:0]             ar_remain_chunks                ;
logic    [AXI_MAX_BURST_LG2-1:0]             ar_remain_tail                  ;
...
logic                                        ar_row_last_burst               ;
logic    [8:0]                               ar_issue_beats                  ;
logic                                        ar_issue_row_first              ;
logic                                        ar_issue_row_last               ;
logic                                        ar_issue_row_single             ;
logic    [BURST_ALIGN_BITWID-1:0]            ar_issue_head_offset            ;
logic    [BURST_ALIGN_BITWID-1:0]            ar_issue_tail_bytes             ;
logic                                        ar_issue_fire                   ;
...
always_ff @(posedge clk or negedge rst_n) begin
    if(rst_n == 0)begin
        ...
        mvout_col_num_q         <= '0;
        mvout_row_num_q         <= '0;
        cfg_mvin_sram_stride_q  <= '0;
        cfg_mvin_dram_stride_q  <= '0;
        cfg_mvout_sram_stride_q <= '0;
        cfg_mvout_dram_stride_q <= '0;
        mvin_rows_real_q        <= '0;
        mvin_cols_real_q        <= '0;
        mvout_rows_real_q       <= '0;
        mvout_cols_real_q       <= '0;
        mvin_contiguous_elems_q <= '0;
        mvout_contiguous_elems_q <= '0;
        mvin_contiguous_2d_q    <= 1'b0;
        mvout_contiguous_2d_q   <= 1'b0;
    end
    else begin
        if(dma_mvin_req_en)begin
            mvin_col_num_q         <= mvin_col_num;
            mvin_row_num_q         <= mvin_row_num;
            cfg_mvin_sram_stride_q <= cfg_mvin_sram_stride;
            cfg_mvin_dram_stride_q <= cfg_mvin_dram_stride;
            mvin_rows_real_q       <= mvin_rows_real;
            mvin_cols_real_q       <= mvin_cols_real;
            mvin_contiguous_elems_q <= mvin_rows_real * mvin_cols_real;
            mvin_contiguous_2d_q   <= mvin_contiguous_2d;
        end
        if(dma_mvout_req_en)begin
            mvout_col_num_q         <= mvout_col_num;
            mvout_row_num_q         <= mvout_row_num;
            cfg_mvout_sram_stride_q <= cfg_mvout_sram_stride;
            cfg_mvout_dram_stride_q <= cfg_mvout_dram_stride;
            mvout_rows_real_q       <= mvout_rows_real;
            mvout_cols_real_q       <= mvout_cols_real;
            mvout_contiguous_elems_q <= mvout_rows_real * mvout_cols_real;
            mvout_contiguous_2d_q   <= mvout_contiguous_2d;
        end
    end
end
...
assign mvout_acc_fp32_mode_req = cfg_mvout_source &&
                                 ((cfg_mvout_output_precision == 2'b01) ||
                                  (cfg_mvout_output_precision == 2'b11));
...
always_ff @(posedge clk or negedge rst_n) begin
    ...
    else if(aw_row_last_burst && m_axi_awvalid && m_axi_awready)  //each row finish,addr=addr+stride
...
always_ff @(posedge clk or negedge rst_n) begin
    if(rst_n == 0)
    m_axi_awvalid <= 0;
    else if(aw_row_last_burst && m_axi_awready && m_axi_awvalid) //each row ,the last handshake finish,valid=0
    m_axi_awvalid<= 0;
...
always_ff @(posedge clk or negedge rst_n) begin
    ...
    awaddr_row_first <= 0;
    else if(aw_row_last_burst && m_axi_awvalid && m_axi_awready)  //row change ,cal the next row first addr
...
always_comb begin
    ...
    aw_row_last_burst       = (aw_remain_beats <= BEATS_WIDTH'(AXI_MAX_BURST_BEATS)) &&
                              (aw_remain_beats <= BEATS_WIDTH'(aw_beats_to_4k));
...
always_comb begin
 ...
 acc_rd_addr  = cfg_mvout_source == 1 ? sram_rd_addr[ACC_ADDR_WIDTH-1:0] : '0;
	...
	//---------------------------------------------
	assign ar_issue_fire = m_axi_arvalid && m_axi_arready;
	...
	always_ff @(posedge clk or negedge rst_n) begin
	    ...
	    m_axi_araddr <= {mvin_dram_addr[RF_DATA_WIDTH/2-1:BURST_ALIGN_BITWID],{(BURST_ALIGN_BITWID){1'b0}}};
		    else if(ar_issue_row_last && ar_issue_fire)
		    m_axi_araddr <= (((araddr_row_first + reg_cfg_mvin_dram_stride )>>BURST_ALIGN_BITWID)<<BURST_ALIGN_BITWID);
		    else if(ar_issue_fire)
		    m_axi_araddr <= m_axi_araddr + ar_issue_beats*BURST_ADDR_INCR;
		    else
		...
		always_ff @(posedge clk or negedge rst_n) begin
		    ...
		    m_axi_arvalid <= 0;
	    else if(current_raddr_state==S_READ && !reg_dma_mvin_req_en[1] &&
	            ar_remain_beats>0 && r_outstanding_cnt<AXI_RD_OUTSTANDING)
	    m_axi_arvalid <= 1;
	    else
	...
	always_ff @(posedge clk or negedge rst_n) begin
	    if(rst_n == 0) begin
	        m_axi_arlen           <= '0;
	        ar_issue_beats        <= '0;
	        ar_issue_row_first    <= 1'b0;
	        ar_issue_row_last     <= 1'b0;
	        ar_issue_row_single   <= 1'b0;
	        ar_issue_head_offset  <= '0;
	        ar_issue_tail_bytes   <= '0;
	    end
	    else if(!m_axi_arvalid &&
	            current_raddr_state==S_READ && !reg_dma_mvin_req_en[1] &&
	            ar_remain_beats>0 &&
	            r_outstanding_cnt<AXI_RD_OUTSTANDING) begin
	        m_axi_arlen           <= (ar_final_beats>0)?ar_final_beats-1:0;
	        ar_issue_beats        <= ar_final_beats;
	        ar_issue_row_first    <= ar_remain_beats == ar_total_beats;
	        ar_issue_row_last     <= ar_row_last_burst;
	        ar_issue_row_single   <= ar_total_beats == 1;
	        ar_issue_head_offset  <= (ar_remain_beats == ar_total_beats) ? araddr_row_first[BURST_ALIGN_BITWID-1:0] : '0;
	        ar_issue_tail_bytes   <= araddr_row_first[BURST_ALIGN_BITWID-1:0] + reg_read_col_num[BURST_ALIGN_BITWID-1:0];
	    end
	    else begin
	        m_axi_arlen           <= m_axi_arlen;
	        ar_issue_beats        <= ar_issue_beats;
	        ar_issue_row_first    <= ar_issue_row_first;
	        ar_issue_row_last     <= ar_issue_row_last;
	        ar_issue_row_single   <= ar_issue_row_single;
	        ar_issue_head_offset  <= ar_issue_head_offset;
	        ar_issue_tail_bytes   <= ar_issue_tail_bytes;
	    end
	end
always_ff @(posedge clk or negedge rst_n) begin
    if(rst_n==0)
    r_outstanding_cnt<='0;
	        ...
	        case({ar_issue_fire,m_axi_rlast&&m_axi_rvalid&&m_axi_rready})
...
always_ff @(posedge clk or negedge rst_n) begin
	...
	assign ar_remain_beats = {ar_remain_chunks, ar_remain_tail};
	...
	always_ff @(posedge clk or negedge rst_n) begin
	    ...
	    else if(reg_dma_mvin_req_en[1] || current_raddr_state==S_WAIT) begin
	        ar_remain_chunks <= ar_row_beats_calc[BEATS_WIDTH-1:AXI_MAX_BURST_LG2];
	        ar_remain_tail   <= ar_row_beats_calc[AXI_MAX_BURST_LG2-1:0];
	    end
	    else if(ar_issue_fire) begin
	        if(ar_issue_row_last) begin
	            ar_remain_chunks <= '0;
	            ar_remain_tail   <= '0;
	        end
	        else if(ar_issue_beats == AXI_MAX_BURST_BEATS[8:0]) begin
	            ar_remain_chunks <= ar_remain_chunks - 1'b1;
	            ar_remain_tail   <= ar_remain_tail;
	        end
	        else if(ar_remain_tail >= ar_issue_beats[AXI_MAX_BURST_LG2-1:0]) begin
	            ar_remain_chunks <= ar_remain_chunks;
	            ar_remain_tail   <= ar_remain_tail - ar_issue_beats[AXI_MAX_BURST_LG2-1:0];
	        end
	        else begin
	            ar_remain_chunks <= ar_remain_chunks - 1'b1;
	            ar_remain_tail   <= ar_remain_tail + AXI_MAX_BURST_BEATS[AXI_MAX_BURST_LG2-1:0] - ar_issue_beats[AXI_MAX_BURST_LG2-1:0];
	        end
	    end
	end
	always_comb begin
	    ...
	    ar_row_beats_calc      = (araddr_row_first[BURST_ALIGN_BITWID-1:0]+(BURST_ADDR_INCR - 1)+reg_read_col_num)>>BURST_ALIGN_BITWID;
	    ar_total_beats          = ar_row_beats_calc;
    ...
    ar_row_last_burst       = (ar_remain_beats <= BEATS_WIDTH'(AXI_MAX_BURST_BEATS)) &&
                              (ar_remain_beats <= BEATS_WIDTH'(ar_beats_to_4k));
	end

assign rd_meta_push           = ar_issue_fire;
...
always_ff @(posedge clk) begin
    if(rd_meta_push) begin
	        rd_meta_burst_beats[rd_meta_wr_ptr] <= ar_issue_beats;
	        rd_meta_row_first[rd_meta_wr_ptr]   <= ar_issue_row_first;
	        rd_meta_row_last[rd_meta_wr_ptr]    <= ar_issue_row_last;
	        rd_meta_row_single[rd_meta_wr_ptr]  <= ar_issue_row_single;
	        rd_meta_head_offset[rd_meta_wr_ptr] <= ar_issue_head_offset;
	        rd_meta_tail_bytes[rd_meta_wr_ptr]  <= ar_issue_tail_bytes;
	    end
	end
...
always_comb begin
    case (current_waddr_state)
        ...
        S_WRITE: begin
            if(aw_row_last_burst && m_axi_awready && m_axi_awvalid)
...
always_comb begin
    case (current_raddr_state)
        ...
        S_READ: begin
	            if(ar_issue_row_last && ar_issue_fire)
	        ...
	        S_WAIT: begin
	            if(araddr_row_cnt == reg_read_row_num)
	            next_raddr_state = IDLE ;
	            else
	            next_raddr_state = S_READ ;
        end
...
always_ff @(posedge clk or negedge rst_n) begin
    if(rst_n == 0)begin
        ...
        if(mvin_contiguous_2d_q) begin
            reg_cfg_mvin_sram_stride   <=  0 ;
            reg_cfg_mvin_dram_stride   <=  0 ;
            reg_read_row_num           <=  1 ;
            reg_read_col_num           <=  mvin_contiguous_elems_q << mvin_dram_data_size_log2 ;
        end
        else begin
            reg_cfg_mvin_sram_stride   <=  (cfg_mvin_sram_stride_q-mvin_col_num_q-1)<<mvin_sram_data_size_log2  ;
            reg_cfg_mvin_dram_stride   <=  cfg_mvin_dram_stride_q<<mvin_dram_data_size_log2 ;
            reg_read_row_num           <=  mvin_row_num_q+1 ;
            reg_read_col_num           <=  (mvin_col_num_q + 1)<<mvin_dram_data_size_log2 ;
        end
    end
    else if(reg_dma_mvout_req_en[0])begin
        if(mvout_contiguous_2d_q) begin
            reg_cfg_mvout_sram_stride  <=  0 ;
            reg_cfg_mvout_dram_stride  <=  0 ;
            reg_write_row_num          <=  1 ;
            reg_write_col_num          <=  mvout_contiguous_elems_q << mvout_dram_data_size_log2 ;
        end
        else begin
            reg_cfg_mvout_sram_stride  <=  (cfg_mvout_sram_stride_q-mvout_col_num_q-1)<<mvout_sram_data_size_log2 ;
            reg_cfg_mvout_dram_stride  <=  cfg_mvout_dram_stride_q<<mvout_dram_data_size_log2;
            reg_write_row_num          <=  mvout_row_num_q+1        ;
            reg_write_col_num          <=  (mvout_col_num_q + 1)<<mvout_dram_data_size_log2 ;
        end
    end
    else begin
